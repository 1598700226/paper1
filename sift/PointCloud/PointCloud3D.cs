...
namespace sift.PointCloud
{
    public class PointCloud3D
    {
        ...
        public PointCloud3D(double xw, double yw, double zw, int pic_x, int pic_y)
        {
            X = xw;
            Y = yw;
            Z = zw;
        ...
        // 点云降采样
        public static List<PointCloud3D> downSampling(int picWidth, int picHeight, int r, List<PointCloud3D> pointCloud3Ds) {
            ...
            for (int i = 0, j = 0; i < pointCloud3Ds.Count - 1; i++)
            {
                if (pointCloud3Ds[i].H == pointCloud3Ds[i + 1].H)
                {
                    ...
                    if (i == pointCloud3Ds.Count - 2)
                    {
                        ...
                        pic_x = pic_x / (i + 1 - j);
                        pic_y = pic_y / (i + 1 - j);
                        filterPointClouds.Add(new PointCloud3D(x, y, z, pic_x, pic_y));
                    }
                    continue;
                }
                else
